...
pub struct Body {
    ...
    /// The message's HTML alternative, through [`html::restrict`] — what "original formatting"
    /// mode lays out instead of `markdown`. Not the bytes as sent: the same allowlist the Markdown
    /// path reads from, written back as HTML, so the layout engine is handed nothing that names
    /// a resource or a handler. Present whenever the message *has* an HTML part anywhere, not
    /// only when [`pick`] chose to show it: a real plain-text alternative outranks HTML by design
    /// (see `pick`'s own doc comment), but that is a choice about the *default* view, not a claim
    /// that the sender's HTML doesn't exist. `trackers`/`images`/`misleading` above are computed
    /// from whichever text `pick` chose, regardless of mode, so detection doesn't vary by render
...
pub fn body(root: &Part) -> Body {
    ...
    if body.html.is_none() {
        body.html = find_html(root).and_then(|part| part.body.as_deref()).map(html::restrict);
    }
    body
}

/// The first HTML part anywhere in the tree, independent of what [`pick`] chose — see
/// [`Body::html`]. Walks the same containers `pick` does (so a forward's HTML is found too,
...
mod tests {
    ...
    fn a_plain_alternative_winning_does_not_lose_the_html_it_beat() {
        ...
        assert_eq!(body.html.as_deref(), Some("<p>marketing</p>"));
    }
